...
const NEWS_LAYER_IDS = ['event', 'sentiment', 'info_catalyst']

function layerHeadlines(layer) {
  const raw = layer.rawData || {}
  const items = raw.newsItems || raw.eventArticles || raw.articles || []
  const seen = new Set()
  const out = []
  for (const a of items) {
    const title = (a && (a.title || a.llm_summary)) || ''
    if (!title) continue
    const key = title.trim().toLowerCase().slice(0, 100)
    if (seen.has(key)) continue
    seen.add(key)
    out.push({ title, url: a && a.url ? a.url : null })
    if (out.length >= 6) break
  }
  if (out.length === 0 && (raw.topHeadlines || raw.headlineList)) {
    const list = raw.topHeadlines || raw.headlineList
    for (const t of list) {
...
function LayerDetailPanel({ layer, layerIndex }) {
  ...
  const isNewsLayer = NEWS_LAYER_IDS.includes(layer.id)
  const metrics = (layer.rawData && layer.rawData.metrics) || {}
  const headlines = isNewsLayer ? layerHeadlines(layer) : []
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <div>
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: '#404060', letterSpacing: '0.1em', display: 'block', marginBottom: '2px' }}>
                LAYER {layerIndex != null ? layerIndex : (LAYERS.findIndex(l => l.id === layer.id) + 1) || 0} — {layer.shortName}
              </span>
      ...
      {expanded && (
        <div style={{ padding: '12px 16px 16px', borderTop: '1px solid #1e1e35', background: '#111120' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: '#404060', letterSpacing: '0.1em' }}>AI REASONING</span>
            {layer.sources?.llm && (
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '8px', color: '#00ffaa', padding: '1px 6px', borderRadius: '2px', background: 'rgba(0,255,170,0.15)' }}>Generated by Gemini</span>
            )}
          </div>
          <p style={{ fontSize: '12px', color: '#a0a0c0', lineHeight: '1.7' }}>{layer.reasoning}</p>

          {isNewsLayer && Object.keys(metrics).length > 0 && (
            <div style={{ marginTop: '12px' }}>
              <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: '#404060', letterSpacing: '0.1em', marginBottom: '6px' }}>KEY METRICS</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {Object.entries(metrics).map(([k, v]) => (
                  <div key={k} style={{ background: '#0d0d16', padding: '6px 10px', borderRadius: '3px', border: '1px solid #1e1e35' }}>
                    <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '8px', color: '#404060', textTransform: 'uppercase' }}>{String(k).replace(/([A-Z])/g, ' $1').trim()} </span>
                    <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '11px', color: '#a0a0c0' }}>{typeof v === 'number' && (v > 1 || v < -1) ? v.toFixed(2) : String(v)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {isNewsLayer && headlines.length > 0 && (
            <div style={{ marginTop: '12px' }}>
              <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: '#404060', letterSpacing: '0.1em', marginBottom: '6px' }}>TOP HEADLINES</div>
              <ul style={{ margin: 0, paddingLeft: '18px', fontSize: '11px', color: '#a0a0c0', lineHeight: '1.6' }}>
                {headlines.slice(0, 6).map((item, i) => {
                  const title = typeof item === 'string' ? item : (item && item.title) || ''
                  const url = typeof item === 'object' && item && item.url ? item.url : null
                  const displayTitle = (title || '').slice(0, 120) + ((title || '').length > 120 ? '…' : '')
                  return (
                    <li key={i} style={{ marginBottom: '6px' }}>
                      {url ? (
                        <>
                          ...
                          {' '}
                          <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '10px', color: '#4466ff', textDecoration: 'none' }} onClick={e => e.stopPropagation()}>Source →</a>
                        </>
                      ) : (
                        displayTitle
                      )}
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {signals.map((layer, i) => (
            <LayerDetailPanel key={layer.id} layer={layer} layerIndex={i + 1} />
          ))}
        </div>
